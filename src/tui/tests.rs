...
fn root_export_writes_sfv_from_tui_action() {
    ...
    export_selected_root_sfv(&conn, Some(&root), &mut state).unwrap();

    let sfv_path = temp.path().join("root.sfv");
    let sfv = std::fs::read_to_string(&sfv_path).unwrap();
    assert!(sfv.contains("; Generated by gremlin from metadata"));
    assert!(sfv.contains("hello world.txt a17c9e42"));
    assert!(state.status.contains("exported SFV"));
