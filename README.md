...
- Transfer planning/copying: persisted dry-run root-to-root plans, job events, CLI inspection, TUI persisted-plan loading, TUI plan browsing/run/review controls, detailed local-copy progress, streamed hash-checked local copy execution, and optional paranoid readback exist for TUI selections; next slices should add review retargeting, checksum collection comparisons, and resumable copy checkpoints.
